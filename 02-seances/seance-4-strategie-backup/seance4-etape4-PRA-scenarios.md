...
### Alternative — Flashback Table (sans arrêt de base)

Quand l'incident concerne **une seule table** et a été détecté rapidement (UNDO encore disponible), Oracle propose une option beaucoup plus rapide qui **ne nécessite pas d'arrêter la base** : `FLASHBACK TABLE`.

#### Quand préférer Flashback Table ?

| Critère | Flashback Table | PITR RMAN |
|---|---|---|
| Périmètre | Une seule table | Toute la base |
| Arrêt de base requis | ❌ Non | ✅ Oui |
| Délai depuis l'incident | Court (UNDO disponible, généralement < 15 min) | Quelconque |
| RTO | ~2 min | ~25 min |
| Prérequis | `ROW MOVEMENT` activé sur la table | Sauvegarde RMAN + archive logs |

> ⚠️ **Limite** : Flashback Table exploite les segments UNDO. Si le paramètre `UNDO_RETENTION` est dépassé (par défaut 900 secondes soit 15 min), les données UNDO sont écrasées et Flashback Table échoue. Dans ce cas, seul le PITR RMAN peut récupérer les données.

#### Procédure Flashback Table

```sql
-- Étape 1 : activer le mouvement de lignes sur la table
-- (requis une seule fois — peut être fait lors de la création de la table)
ALTER TABLE FENETRE_COM ENABLE ROW MOVEMENT;

-- Étape 2 : rembobiner la table à l'instant souhaité
-- (remplacer l'horodatage par 2-3 minutes avant l'incident détecté)
FLASHBACK TABLE FENETRE_COM TO TIMESTAMP
  TO_TIMESTAMP('2026-06-07 14:43:00', 'YYYY-MM-DD HH24:MI:SS');

-- Étape 3 : vérifier la récupération
SELECT COUNT(*) FROM FENETRE_COM;
-- Doit retourner 5

-- Étape 4 (optionnel) : désactiver le mouvement de lignes
ALTER TABLE FENETRE_COM DISABLE ROW MOVEMENT;
```

#### Arbre de décision — Quel mécanisme choisir ?

```
Incident détecté sur FENETRE_COM
          ↓
  Délai < 15 min ET UNDO disponible ?
          ↓
    OUI → FLASHBACK TABLE     ← RTO ~2 min, base reste ouverte
    NON → PITR RMAN           ← RTO ~25 min, arrêt de base requis
```

**RTO Flashback Table : ~2 min** ✅ très largement conforme au contrat (30 min).
...
|---|---|---|---|---|---|
| DELETE erroné (détecté < 15 min) | Erreur opérateur | Flashback Table (`FLASHBACK TABLE … TO TIMESTAMP`) | ~2 min | ✅ 15 min | ✅ Oui |
| DELETE erroné (détecté > 15 min) | Erreur opérateur | RMAN PITR (`RECOVER UNTIL TIME` + `RESETLOGS`) | ~25 min | ✅ 15 min | ✅ Oui |
